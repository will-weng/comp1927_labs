...
static int *lengthAndChain(HashTable ht);
static int max;
...
void HashTableStats(HashTable ht)
{
	...
	for(total = 0, i = 0, counter = 0;total < ht->nslots; i++, counter++
   	printf("%d %d\n", i, ht->lists[counter]);
	}
	*/
...
// creates an array of length and number of chains
static int *lengthAndChain(HashTable ht) {
   ...
   for(i = 0; i < ht->nslots; i++) {
      if(max < ListLength(ht->lists[i]))
         max = ListLength(ht->lists[i]);
   }
   
   int *data = malloc(max+1);
   
   for(i = 0; i <= max; i++)
      data[i] = 0;
   
   for(i = 0; i < ht->nslots; i++) {
      data[ListLength(ht->lists[i])]++;
   }
