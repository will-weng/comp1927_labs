...
int nameToID(char *name)
{
   ...
   */
   ...
   first = 0; last = MAX_MAP_LOCATION;
   middle = (first + last)/2;
   
   while(first <= last) {
      if(strcmp(places[middle].name, name) < 0) {
         first = middle + 1;
      } else if(strcmp(places[middle].name, name) == 0) {
         return places[middle].id;
      } else {
         last = middle - 1;
      }
...
{
   // an attempt to speed up linear search
   Place *p;
