...
int main(int argc, char **argv)
{
	...
	*/
	...
	Graph Links = newGraph(maxURLs);
   Set Seen = newSet();
	
	int num = maxURLs*maxURLs;
	while(emptyQueue(ToDo) == 0 && num > 0) {
	   char *url = leaveQueue(ToDo);
	   char *c;
	   if((c = strstr(url, "unsw")) == NULL) {
	      break;
	   }
